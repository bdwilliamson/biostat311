...
For this activity, we're going to look at the clinical trial data you're analzying on Homework 7. Here's a brief summary of the study:
...
- **Time period:** first enrollment on March 26, 2018 and study ends 3 years later, on March 26, 2021
...
# enter code to load data here
# call your data.frame 'leuk' so that the code chunks below will work 
```
...
Once the `survival` package is loaded, we can use one of its functions (the `Surv` function) to create a time to death variable.
...
# Bonus Question: Perform a proportional hazards regression of time-to-death as the outcome and treatment group as the predictor.

To fit this proportional hazards regression model, run the following code chunk.
