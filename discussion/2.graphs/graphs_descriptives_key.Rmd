...
The `uwIntroStats` package contains functions that compute descriptive statistics easily. Load that package now.
...
First, focus on plotting the `fev` values for pre-teens (uncomment the line of code, which only has a single `#` in front of it, and run it):

```{r boxplot-3, echo = TRUE}
...
Correct one of the issues that you should have identified, by adding some random noise to the `fev$teen` values, so that there is some visual separation between the points (this is also called `r colFmt("jittering", "blue")`). Uncomment the next code chunk (only those lines with single `#` characters at the beginning) and run it; do you think that this has improved the visual layout of the plot? Can you gain a bit more information now than you had before?

```{r boxplot-4, echo = TRUE}
...
Now create an empty plot, using similar code to before, and add the boxplots on top (uncomment and run this code, similar to before):

```{r boxplot-5, echo = TRUE}
...
Finally, uncomment (similar to before) and run the next code chunk. In words, what is each line of code doing?

```{r boxplot-6, echo = TRUE}
## create any empty plot (type = "n"), but give it x- and y-axis limits, and labels
# plot(1, type = "n", xlim = c(-1, 2), ylim = c(0, 6), ylab = "FEV (liters per second)", xlab = "Age Group", axes = FALSE, pch = 16)
## add the boxplots (set add to TRUE, but other things to FALSE since we are adding)
# boxplot(fev$fev ~ fev$teen, xlab = "", ylab = "", add = TRUE, at = c(0, 1), axes = FALSE, outline = FALSE)
...
To understand loess smooths, first you have to understand best fitting lines. Best fitting lines minimize the squared difference between each data point and the line: the idea is that the best fitting line should not be too far away from any of the data points.
...
*Replace this sentence with your description, and the answer to the final question.*
