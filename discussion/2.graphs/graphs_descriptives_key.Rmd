...
Now that you have a variable that differentiates teens from pre-teens, you can re-create the boxplot from slide 27 with the following code: `boxplot(fev$fev ~ fev$teen, xlab = "Age Group", ylab = "FEV (liters per second)", names = c("Pre-teen (< 13)", "Teen (13 +)"))`. Here's what each component of that line of code means, in English:
...
boxplot(fev$fev ~ fev$teen, xlab = "Age Group", ylab = "FEV (liters per second)", names = c("Pre-teen (< 13)", "Teen (13 +)"))
```
...
plot(fev$teen[fev$teen == 0], fev$fev[fev$teen == 0], xlim = c(-1, 2), ylim = c(0, 6), ylab = "FEV (liters per second)", xlab = "Age Group", axes = FALSE, pch = 16)
...
## plot the fev values for pre-teens only
plot(pre_teen_noisy, fev$fev[fev$teen == 0], xlim = c(-1, 2), ylim = c(0, 6), ylab = "FEV (liters per second)", xlab = "Age Group", axes = FALSE, pch = 16)
...
plot(1, type = "n", xlim = c(-1, 2), ylim = c(0, 6), ylab = "FEV (liters per second)", xlab = "Age Group", axes = FALSE, pch = 16)
## add the boxplots (set add to TRUE, but other things to FALSE since we are adding)
boxplot(fev$fev ~ fev$teen, xlab = "", ylab = "", add = TRUE, at = c(0, 1), axes = FALSE, outline = FALSE)
```
...
plot(fev$age[fev$smoker == 0], fev$fev[fev$smoker == 0], xlab = "Age (years)", ylab = "FEV (liters per second)", main = "FEV versus age, stratified by smoking status", pch = 16, col = "blue")
## add in code to add the points for smokers, colored red (Hint: use the points() function)
# note that using "subsest" is equivalent to using the brackets as I did above
points(subset(fev$age, fev$smoker == 1), subset(fev$fev, fev$smoker == 1), col = "red", pch = 18)

## add a legend
legend("topleft", legend = c("Smoker", "Nonsmoker"), col = c("red", "blue"), pch = c(18, 16))
```
...
In the previous section, you plotted FEV versus age and stratified by smoking status. In the final component, you described the trends in FEV with age for smokers and nonsmokers. 

It is a bit difficult to describe these trends based on the data alone. `r colFmt("Trend lines", "blue")` (e.g., lines of best fit) can help to visualize these trends. Today, you'll fit a particular type of trend line, called a `r colFmt("loess smooth", "blue")`.

To understand loess smooths, first you have to understand best fitting lines. Best fitting lines minimize the squared difference between each data point and the line: the idea is that the best fitting line should not be too far away from any of the data points. If you draw a vertical line from each data point to the line, and square it (this makes sure that being above and below the line don't cancel each other out), you have a measure of the distance between the line and each data point. Making this difference small means that the line describes the data fairly well.
...
I have reproduced the scatter plot from before, and added a loess smooth for the nonsmokers. Copy your code that adds the points for smokers into the next code chunk, and add code to make a loess smooth for smokers. Make sure to color the line for smokers red!

```{r scatter-2, eval = TRUE}
## plot FEV values for all ages, among nonsmokers
plot(fev$age[fev$smoker == 0], fev$fev[fev$smoker == 0], xlab = "Age (years)", ylab = "FEV (liters per second)", main = "FEV versus age, stratified by smoking status", pch = 16, col = "blue")
## add a loess smooth
nonsmoker_loess <- lowess(fev$age[fev$smoker == 0], fev$fev[fev$smoker == 0])
lines(nonsmoker_loess, col = "blue", lty = 2)
## add in code to add the points for smokers, colored red (Hint: use the points() function)
# note that using "subsest" is equivalent to using the brackets as I did above
points(subset(fev$age, fev$smoker == 1), subset(fev$fev, fev$smoker == 1), col = "red", pch = 18)
## add in code to add the loess smooth for smokers, colored red
smoker_loess <- lowess(subset(fev$age, fev$smoker == 1), subset(fev$fev, fev$smoker == 1))
lines(smoker_loess, col = "red", lty = 2)

## add a legend
legend("topleft", legend = c("Smoker", "Nonsmoker"), col = c("red", "blue"), pch = c(18, 16))
```

Based on this plot, describe the trends in FEV with age for smokers and nonsmokers, separately. Additionally, describe the difference in the trends in FEV with age between smokers and nonsmokers. How do the loess smooths make answering these questions easier? 

*FEV tends to increase with age, for nonsmokers. When we restrict our attention to smokers, it seems that FEV still increases with age, but the increase in FEV is much smaller.*

*There appears to be a different relationship between FEV and age for smokers and nonsmokers. For nonsmokers, FEV appears to increase dramatically with age. For smokers, this increase seems to be a much smaller magnitude.*

*The loess smooths made answering these questions much easier for me. Looking at the slopes of the trend lines (i.e., how quickly are things changing) alerted me to the fact that FEV isn't actually decreasing with age in smokers. This is an interesting finding, and one that we'll explore more!*

*We've seen before that our brains can only comprehend so much information. By adding trend lines to this scatterplot, you make the story easier for your reader to understand. Always try to make it as easy as possible for your reader to reach the conclusion you want them to reach!*