...
## run linear regression with log(age) as outcome
...
As we discussed in class yesterday, a third option is to use log base 2 (i.e., $\log_2$) to transform our predictor rather than to use the natural log and have to interpret $\log_e(2)\beta_1$ rather than $\beta_1$ as we did above. Let's try fitting this model with $\log_2(\text{age})$ now. 
