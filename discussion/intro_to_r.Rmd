...
You access values in vectors using open and closed square brackets `[]`. Youaccess values in data frames using the same square brackets---however, since data frames have rows and columns, you access a value using `[<replace row number here>, <replace with column number>]` (without the `<>` symbols!). 

For example, access the 5th element in the 3rd column of the FEV data:
...
## we could also do ^ for smoker, but I'll show you a different function
fev$smoker <- ifelse(fev$smoke == 2, 1, 0)
```

This made use of the fact that I knew that `2` in the original data meant female and smoker, respectively. Thus I could either subtract one from each observation, or use `ifelse()`: this takes a logical comparison (e.g., is `smoke` equal to 2?), what to assign if the expression is true (e.g., `1`), and what to assign if the expression is false (e.g., `0`).

Also, we created variables by just using `$` combined with a new variable name!
