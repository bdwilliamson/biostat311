...
| `ttest` | Run a t-test. | `ttest(fev$fev, by=fev$sex, var.eq=FALSE)` | Runs a two-sample t-test. The first argument specifies the outcome (the variable `fev` in the dataset called `fev`), and the `by=` argument specifies which variable defines the two groups we want to compare. The final argument, `var.eq=FALSE` says "do not run the equal variances" option (i.e., *do* run the unequal variances option). | Chapter 0 slides, slide 0.55 | 