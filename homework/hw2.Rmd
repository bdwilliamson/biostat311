...
## d. (Optional, 0 points) Linear regression with a transformed predictor
Suppose we first computed $\bar{age}$ (the average age in the dataset) and then subtracted it from the observed ages, fitting the model: $$E[Mar|age] = \beta_0 + \beta_1 \times (age - \bar{age}).$$ Repeat parts (a) and (b) with this modificaiton in mind.
...
Once you have completed your assignment, update and run the `define-file-names` and `extract-code` code chunks (if needed) to extract the R code from this .Rmd file and send it to a .R file. 
...
>>>>>>> 9d1e2710e41afe37b70034564eca0fb6234cd713
