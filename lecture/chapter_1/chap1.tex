...
\item \textbf{Robust standard errors}: allow the population variance of $Y$ to differ across values of $X$ % allow for heteroskedasticity
	\begin{itemize}
	\item Appropriate if the variances differ, or if they're constant!\pause 
	\end{itemize}
\end{itemize}
\end{frame}
...
\pause
\begin{small}
Recall the definition of \textbf{probability}: the relative frequency of an event in the long run (i.e., if an experiment is repeated many times, what proportion of those times does the event occur?)\\
...
\item Perform \textit{statistical inference}:
	\begin{itemize}
	\item Calculate the corresponding \textbf{statistic}: difference in sample means comparing two groups that differ by one year in age = \textit{linear regression slope} \pause
...
% show R output
\begin{frame}
\hspace*{-1cm} \includegraphics[width=0.99\paperwidth]{./plots/regress_fev_vs_age}
\end{frame}

% highlight what's important
\begin{frame}
\hspace*{-1cm} \includegraphics[width=0.99\paperwidth]{./plots/regress_fev_vs_age_highlights}
\end{frame}
...
% show R output
\begin{frame}
\hspace*{-0.5cm} \includegraphics[width=0.9\paperwidth]{./plots/regress_fev_vs_sex} % ask the to id coef, CI, p-val
\end{frame}
...
\begin{frame}
\frametitle{Least squares estimation}
\textit{Least squares:} minimize the sum of the squared distances from the observed points to the fitted line.\vspace{-0.5cm}

\center \includegraphics[height=0.8\textheight]{./plots/linear-regr-ls}
\end{frame}
