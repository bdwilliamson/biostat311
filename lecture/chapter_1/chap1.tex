...
\begin{frame}
\frametitle{Warm-up: interpreting coefficients}
How do we interpret $\beta_0, \beta_1$ in this linear regression model? $$E[\text{FEV}|\text{Age}] = \beta_0 + \beta_1 \text{Age}$$\pause

$\beta_0$: average FEV (l/sec) among newborn children \pause
\begin{align*}
\color{blue} E[\text{FEV}|\text{Age} = 0] \color{black} = \beta_0 + \beta_1 (0) =  \color{blue} \beta_0 \color{black} 
\end{align*}\pause

$\beta_1$: difference in average FEV (l/sec) comparing two groups that differ in age by one year \pause
\begin{align*}
\color{blue} E[\text{FEV}|\text{Age} = x+1] & \color{blue} - E[\text{FEV}|\text{Age} = x]\color{black} = \\
\left[\beta_0 + \beta_1 (x+1)\right] &- \left[\beta_0 + \beta_1 (x) \right] = \\
\left[\beta_0 + \beta_1 (x) + \beta_1 \right] &- \left[\beta_0 + \beta_1(x)\right] =\\
 &\color{blue}\beta_1
\end{align*}
\end{frame}
...
\begin{frame}
\frametitle{Interpreting coefficients: warm-up}
How do we interpret $\beta_0$ in this linear regression model? $$E[\text{FEV}|(\text{Age}-3)] = \beta_0 + \beta_1 \left(\text{Age}-3\right)$$\pause
...
\begin{frame}
\frametitle{Interpreting coefficients: warm-up}
How do we interpret $\beta_1$ in this linear regression model? $$E[\text{FEV}|(\text{Age}-3)] = \beta_0 + \beta_1 \left(\text{Age}-3\right)$$\pause
...
\item Interpret $\beta_0, \beta_1$ in the following models:
	...
	\item $E[\text{FEV}|\text{Age}] = \beta_0 + \beta_1 \text{Age}$ \\\begin{scriptsize}(Don't forget units: FEV = liters/second, Age = years)\end{scriptsize}
...
\textit{Linear transformation:} adding, subtracting, multiplying, and/or dividing by some constant

Which transformations change our interpretation of $\beta_0$?\vspace{-0.3cm}\pause
\begin{itemize}
...
\begin{itemize}
\item Make interpretation of $\beta_0$ more scientifically meaningful (e.g., so we're not talking about the average FEV among people with height 0")
\item Make estimate of $\beta_0$ more trustworhty (e.g., so we're not estimating the average FEV among newborns if all our data were collected on senior citizens)
\item Change units (e.g., difference in FEV in liters/minute comparing two groups that differ in age by one year)\pause
\end{itemize}
...
% both
\begin{frame}
\frametitle{Transformations: $\log(X), \log(Y)$}
...
% why do this? account for non-linearity, scientific motivation
% not that important if you just care about association, also makes interpretation very challenging!
% more important if you care about prediction
...
% sometimes it makes sense to re-code as numbers: binary, genotype
% even if there is a natural order, hard to know what values to assign
% when there is no natural order, even harder!
% what do we do? use dummy variables
