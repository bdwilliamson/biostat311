...
$$\color{blue} E[Y|X] \color{black} = \color{orange} \beta_0 \color{black} + \color{red} \beta_1 \color{amber} \ X$$ \vspace{-0.6cm}

\color{blue}The average of the response, given the value of the predictor, \color{black} is \color{orange} the intercept \color{black} plus \color{red} the slope \color{black} times \color{amber} the value of the predictor.\color{black} \pause
...
\item Identify potential variables that help \textcolor{ForestGreen}{reduce the variability} of our estimates.
...
So far, we have considered the relationship between the outcome, $Y$, and a \textcolor{ForestGreen}{single} predictor of interest, $X$.
...
\item providing information that \textcolor{ForestGreen}{reduces the variability} of our estimates
\end{itemize} 
\end{frame}
...
What happened when we \textcolor{ForestGreen}{did not account for} $X_2$ in the analysis? \pause {\small Hint: we estimated the \textbf{pooled} coefficient for $X_1$ to be 1.89}
\end{frame}
...
\textcolor{red}{Confounders} (slides 1.66--1.71), \textcolor{blue}{effect modifiers} (slides 1.72--1.77), and \textcolor{ForestGreen}{precision variables} (slides 1.77--1.83) all show up in other types of regression as well.
...
\textcolor{ForestGreen}{Precision variable}: associated with the outcome in the population; \textbf{not associated with} the predictor of interest \textbf{in the sample}.
...
\begin{itemize}
\item reduce the variability in our estimates \textcolor{ForestGreen}{(precision variables)}
...
\item \textcolor{ForestGreen}{precision variables}
\end{itemize}
...
\textcolor{ForestGreen}{Precision variables} (associated with the outcome in the population, not associated with the predictor of interest in the sample), in contrast, do not need to be adjusted for.

Adjusting for precision variables tends to \textcolor{ForestGreen}{reduce the standard error for the coefficient of interest} (in linear regression): intuitively, we do a better job explaining the outcome, so we reduce the variability in the problem. 
...
With both \textcolor{red}{confounders} and \textcolor{ForestGreen}{precision variables}, the association between the predictor of interest and the response \textbf{is the same} regardless of the value of the confounder/precision variable.
...
\item[]
\item \textcolor{ForestGreen}{Obtain predicted values from a linear regression analysis} by hand and using R
\item[]
...
Whether or not we need to adjust for other variables (\textcolor{red}{to deal with potential confounding}, \textcolor{ForestGreen}{or increase precision}), or include interaction terms (\textcolor{blue}{for effect modification}) depends on the scientific question, which is aimed at these \textbf{associations}.
...
Additionally, \textcolor{ForestGreen}{the fitted values are our predictions of the mean outcome for the current study participants.}
\end{frame}
...
What is our best estimate of the FEV for a \textcolor{blue}{nonsmoking} \textcolor{ForestGreen}{female} who is \textcolor{magenta}{60 inches tall}? \pause
\begin{align*}
...
\vspace{-0.8cm}
What is our best estimate of the FEV for a \textcolor{blue}{smoking} \textcolor{ForestGreen}{male} who is \textcolor{magenta}{72 inches tall}? \pause
\begin{align*}
...
Making good predictions is a hot topic in statistics currently, and \textcolor{ForestGreen}{``machine learning'' methods are often the best predictive algorithms}; however, \textcolor{blue}{in many settings linear regression can be used to predict well, and is often more interpretable than ``black box'' methods.}
\end{frame}

\section{Assumptions and Diagnostics}
\begin{frame}
\frametitle{{\fontsize{15pt}{7.2}\selectfont SECTION 3.5: ASSUMPTIONS AND DIAGNOSTICS FOR LINEAR REGRESSION}}
\framesubtitle{{\small by Arjun Sondhi, Graduate Student in Biostatistics}}
By the end of Section 2.5, you should be able to:
\begin{itemize}
\item describe the \textbf{necessary} assumptions for valid estimates and inference in linear regression
\item \textbf{contrast} the truly necessary assumptions with the assumptions commonly believed to be necessary
\item interpret diagnostic plots
\item create diagnostic plots in \texttt{R}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{A2) \(\epsilon_i\) are independent of \(X_i\)}
\centering
\includegraphics[width=0.75\textheight]{plots/assumptions_plot_1.png}
\end{frame}

\begin{frame}
\frametitle{A2) \(\epsilon_i\) are independent of \(X_i\)}
\centering
\includegraphics[width=0.75\textheight]{plots/assumptions_plot_2.png}
\end{frame}

\begin{frame}
\frametitle{A2) \(\epsilon_i\) are independent of \(X_i\)}
\centering
\includegraphics[width=0.75\textheight]{plots/assumptions_plot_3.png}
\end{frame}
...
\centering
\includegraphics[width=0.9\textwidth]{plots/hetero.png}
...
\begin{frame}{A4) Full rank predictor matrix}
\centering
\includegraphics[height=0.8\textheight]{plots/assumptions_plot_4.png}
\end{frame}
...
\includegraphics[width=0.6\textwidth]{plots/linear-regr-ls.jpg}
...
\centering
\includegraphics[height=0.8\textheight]{plots/diagnostics_plot_1.png}
\end{frame}

\begin{frame}[fragile]{Residuals vs.~fitted values}
\centering
\includegraphics[height=0.8\textheight]{plots/diagnostics_plot_2.png}
\end{frame}

\begin{frame}[fragile]{Residuals vs.~fitted values}
\centering
\includegraphics[height=0.8\textheight]{plots/diagnostics_plot_3.png}
\end{frame}
...
\begin{frame}{Residual Q-Q plot}
\centering
\includegraphics[height=0.8\textheight]{plots/diagnostics_plot_4.png}
\end{frame}

\begin{frame}{Residual Q-Q plot}
\centering
\includegraphics[height=0.8\textheight]{plots/diagnostics_plot_5.png}
\end{frame}

\begin{frame}{Residual Q-Q plot}
\centering
\includegraphics[height=0.8\textheight]{plots/diagnostics_plot_6.png}
\end{frame}
...
\begin{frame}{Residual ACF plot}
\centering
\includegraphics[height=0.8\textheight]{plots/diagnostics_plot_7.png}
\end{frame}
...
Depending on your scientific question and your study design, you may need to adjust for \textcolor{red}{potential confounders} and/or \textcolor{ForestGreen}{precision variables}, and you may include interaction terms to test \textcolor{blue}{effect modification}.
...
\item[]
\item Draw a \textcolor{ForestGreen}{causal diagram}
\item[]
...
\textcolor{ForestGreen}{Causal diagram: } {\small (space intentionally left blank)} \vspace{5cm}
...
\begin{frame}
\frametitle{Linear regression: inflammatory biomarkers}
\hspace*{-0.5cm}\includegraphics[width = 1.1\textwidth]{plots/inflamm_fib_vs_crp_hidden.png} % CHANGE BACK AFTER ACTIVITY
\end{frame}
...
\textcolor{ForestGreen}{Based on a 95\% confidence interval, this result would not be judged unusual if the true difference in mean blood fibrinogen level were between 4.55 and 5.80 mg/dL} comparing two groups differing in blood CRP level by one mg/L, but having the same age, sex, cholesterol level, BMI, and previous history of cardiovascular disease. \textcolor{orange}{These data provide evidence that the difference in mean blood fibrinogen level is statistically significantly different from zero (p-value $< 0.001$)}. \textcolor{blue}{Based on these results, we have evidence to suggest that blood levels of CRP are associated with blood levels of fibrinogen, adjusted for age, sex, cholesterol, BMI, and previous history of cardiovascular disease.}
\end{frame}
...
\textcolor{ForestGreen}{Based on a 95\% confidence interval, this estimate would not be judged unusual if the true difference in the difference in mean fibrinogen level} comparing two groups differing in CRP by one mg/L but with the same age, sex, BMI, and previous history of cardiovascular disease, comparing groups differing in cholesterol by one mg/dL, \textcolor{ForestGreen}{were between 0.004 and 0.03 mg/dL}. \textcolor{orange}{These data provide evidence that the difference in the difference in mean blood fibrinogen level is statistically significantly different from zero (p-value $= 0.011$)}. \textcolor{blue}{Based on these results, we have evidence to suggest that cholesterol level modifies the association between CRP and fibrinogen, adjusted for age, sex, BMI, and previous history of cardiovascular disease.} 
\end{frame}
