...
\begin{frame}
\frametitle{Prediction}
We can also use logistic regression for prediction. $$\log\left(\text{Odds}[\text{Diabetes}| \text{Male}, \text{Age}]\right) = \beta_0 + \beta_1 \text{Male} + \beta_2 \text{BMI}$$
...
\pause
\begin{scriptsize}
(Remember, our linear regression predictions from HW5 were 0.209 and -0.010.)
\end{scriptsize}
...
$$g\left(E[Y|X_1,\cdots,X_p]\right) = \beta_0 + \beta_1 X_1 + \cdots \beta_pX_p$$

% general formulation
\vspace{-0.2cm}
\begin{itemize}
\item \textbf{Linear regression:} $g(x) = x$ $$E[Y|X_1,\cdots,X_p] = \beta_0 + \beta_1 X_1 + \cdots \beta_pX_p$$
...
Here, $g(x) = \log(x)$, so the regression model takes the form $$\log\left(E[Y|X_1,\cdots,X_p]\right) = \beta_0 + \beta_1 X_1 + \cdots \beta_pX_p$$

When $Y$ is binary, this is the same as $$\log\left(\text{Prob}[Y=1|X_1,\cdots,X_p]\right) = \beta_0 + \beta_1 X_1 + \cdots \beta_pX_p$$

\vspace{-0.1cm}
Interpretation:\vspace{-0.3cm}
\begin{itemize}
\item $e^{\beta_0}$: probability that $Y=1$ when $X_1 = \cdots X_p = 0$
...
A new example: case-control studies

% key message: can't use lin or poisson reg here, only log reg works! this is part of why log reg is so popular
