...
Description: \vspace{-0.3cm}
\begin{itemize}
\item \textit{Sample individuals based on the outcome} (some with, some without), look back in time (usually) for exposure status % when you look back in time = retrospective case-control study
\end{itemize}
...
\textit{Why is it so important to know how to summarize data?} One answer: because we can't look at all of it, but we still want to make sense of it/draw conclusions from it.
...
It is often helpful to present descriptive statistics stratified by your exposure or outcome in ``Table 1."
